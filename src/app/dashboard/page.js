...
// Import services (but NOT context - PageLayout handles that)
import { useBugTrackingMetrics } from '../../services/bugTrackingService';

const Dashboard = ({ suites, onRefreshSuites, suiteLoading, suiteError }) => {
    ...
    useEffect(() => {
        ...
        if (autoRefresh) {
            refreshIntervalRef.current = setInterval(() => {
                ...
                // Only refresh suites if we have the function available
                if (onRefreshSuites) {
                    onRefreshSuites();
                }
            }, 30000);
    ...
    }, [autoRefresh, bugRefetch, onRefreshSuites]);
    ...
    const handleRefresh = useCallback(() => {
        ...
        setCurrentTime(new Date());
        // Let PageLayout handle suite refreshing
        if (onRefreshSuites) {
            onRefreshSuites();
        }
    }, [bugRefetch, onRefreshSuites]);
    ...
    // Determine if we're loading (either bug data or suite data)
    const isLoading = bugLoading || suiteLoading;

    // Create toolbar for PageLayout
    const toolbar = (
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex items-center">
                            ...
                            <p className="text-yellow-800">
                                Warning: {bugError ? 'Bug tracking data may be outdated' : 'Suite data may be outdated'} due to connection issues.
                                <button
                                    onClick={handleRefresh}
