...
const BugTrackerPage = () => {
    ...
    const [optimisticBugs, setOptimisticBugs] = useState([]); // For immediate UI updates
    ...
    // Initialize optimistic bugs when real bugs change
    useEffect(() => {
        if (bugsHook.bugs && Array.isArray(bugsHook.bugs)) {
            setOptimisticBugs([...bugsHook.bugs]);
        }
    }, [bugsHook.bugs]);

    // FIXED: Better filter management with optimistic updates
    useEffect(() => {
        // Use optimistic bugs for immediate UI updates, fallback to real bugs
        const bugsToFilter = optimisticBugs.length > 0 ? optimisticBugs : (bugsHook.bugs || []);
        
        if (bugsToFilter.length >= 0) {
            const newFilteredBugs = applyFilters(bugsToFilter, filters);
            setFilteredBugs(newFilteredBugs);
        }
    }, [optimisticBugs, bugsHook.bugs, filters, applyFilters]);
    ...
    const handleFiltersChange = useCallback((newFilters) => {
        ...
        if (isFiltersCleared) {
            const bugsToUse = optimisticBugs.length > 0 ? optimisticBugs : (bugsHook.bugs || []);
            setFilteredBugs([...bugsToUse]);
        }
    }, [optimisticBugs, bugsHook.bugs]);

    const handleError = useCallback((error, context) => {
        console.error(`Error in ${context}:`, error);
        
        // Don't show error notifications for minor update issues that shouldn't interrupt the user
        if (context === 'update bug' && error?.message?.includes('optimistic')) {
            return; // Skip notification for optimistic update failures
        }
        ...
        uiHook.addNotification?.({
            ...
            duration: 5000, // Changed from persistent to auto-dismiss
        });
    }, [uiHook.addNotification]);

    // FIXED: Optimistic update function that updates UI immediately
    const handleUpdateBug = useCallback(async (bugId, updates) => {
        // Early validation - fail fast without side effects
        if (!bugId) {
            console.error('❌ Bug ID is required for update');
            return { success: false, error: { message: 'Bug ID is required' } };
        }

        if (!updates || typeof updates !== 'object') {
            console.error('❌ Update data is required and must be an object');
            return { success: false, error: { message: 'Invalid update data' } };
        }

        if (bugsHook.bugsLocked) {
            console.error('❌ Bugs are locked');
            return { success: false, error: { message: 'Bugs are locked' } };
        }
        ...
        if (!bugsHook.activeSuite?.id) {
            console.error('❌ No active test suite selected');
            return { success: false, error: { message: 'No active test suite' } };
        }
        ...
        // Find and update the bug optimistically
        setOptimisticBugs(prevBugs => {
            const updatedBugs = prevBugs.map(bug => 
                bug.id === bugId 
                    ? { ...bug, ...optimisticUpdate }
                    : bug
            );
            return updatedBugs;
        });
        ...
        if (selectedBug && selectedBug.id === bugId) {
            setSelectedBug(prev => ({ ...prev, ...optimisticUpdate }));
        }

        console.log('🔄 Optimistic update applied for bug:', bugId);

        try {
            // Call the actual update function in the background
            const result = await bugsHook.updateBug(bugId, optimisticUpdate);

            if (result?.success) {
                console.log('✅ Bug updated successfully on server');
                
                // Only show success notification for significant updates (not inline edits)
                ...
                // The real data will come through bugsHook.bugs and sync with our optimistic state
                return result;
            } else {
                // Revert optimistic update on failure
                console.error('❌ Server update failed, reverting optimistic update');
                
                setOptimisticBugs(prevBugs => {
                    // Find the original bug data to revert to
                    const originalBug = bugsHook.bugs?.find(bug => bug.id === bugId);
                    if (originalBug) {
                        return prevBugs.map(bug => 
                            bug.id === bugId ? originalBug : bug
                        );
                    }
                    return prevBugs;
                });

                // Revert selectedBug too
                if (selectedBug && selectedBug.id === bugId) {
                    const originalBug = bugsHook.bugs?.find(bug => bug.id === bugId);
                    if (originalBug) {
                        setSelectedBug(originalBug);
                    }
                }

                const errorMessage = result?.error?.message || 'Failed to update bug';
                handleError({ message: errorMessage }, 'update bug');
                
                return result || { success: false, error: { message: errorMessage } };
            }

        } catch (error) {
            console.error('❌ Exception during bug update, reverting optimistic update:', error);
            ...
            let userMessage = 'Failed to update bug';
            
            if (error.message?.includes('network')) {
                userMessage = 'Network error - please check your connection';
            } else if (error.message?.includes('permission') || error.message?.includes('unauthorized')) {
                userMessage = 'You do not have permission to update this bug';
            } else if (error.message?.includes('not found')) {
                userMessage = 'Bug not found - it may have been deleted';
            }

            handleError({ message: userMessage }, 'update bug');
    ...
    }, [bugsHook.updateBug, bugsHook.bugsLocked, bugsHook.activeSuite, bugsHook.bugs, selectedBug, uiHook.addNotification, handleError]);
    ...
    const handleSaveBug = useCallback(async (bugData) => {
        try {
            ...
            if (selectedBug) {
                ...
                // For new bugs, add optimistically too
                const newBug = {
                    ...bugData,
                ...
                setOptimisticBugs(prev => [...prev, newBug]);
                ...
                if (result?.success) {
                    ...
                    // Remove temp bug and let real bug data flow through bugsHook.bugs
                    setOptimisticBugs(prev => prev.filter(bug => bug.id !== newBug.id));
                } else {
                    // Remove failed temp bug
                    setOptimisticBugs(prev => prev.filter(bug => bug.id !== newBug.id));
    ...
    }, [bugsHook.bugsLocked, bugsHook.createBug, selectedBug, handleUpdateBug, uiHook.addNotification, handleError]);
    ...
    const handleBulkAction = useCallback(async (action, selectedIds) => {
        try {
            ...
            if (action === 'delete') {
                ...
                // Process bulk updates sequentially to avoid overwhelming the system
                for (const id of selectedIds) {
                    await handleUpdateBug(id, {
                        status: action,
                        updated_at: timestamp,
                    });
                }
