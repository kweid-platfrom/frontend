...
import { Video, Clock, HardDrive, Users, TrendingUp, TrendingDown, Activity, Link, Upload, AlertTriangle } from 'lucide-react';
import { useDashboard } from '../../hooks/useDashboard';
import { useMetricsProcessor } from '../../hooks/useMetricsProcessor';


const RecordingMetrics = ({ filters = {} }) => {
    const { metrics: rawMetrics, loading, error } = useDashboard();
    const metrics = useMetricsProcessor(rawMetrics);

    // Ensure fallback to 0 or empty if undefined
    const totalRecordings = metrics.totalRecordings ?? 0;
    const avgRecordingDuration = metrics.avgRecordingDuration ?? 0;
    const totalRecordingHours = metrics.totalRecordingHours ?? 0;
    const recordingsWithNetworkData = metrics.recordingsWithNetworkData ?? 0;
    const recordingsWithConsoleData = metrics.recordingsWithConsoleData ?? 0;
    const recordingsLinkedToBugs = metrics.recordingsLinkedToBugs ?? 0;
    const recordingsLinkedToTestCases = metrics.recordingsLinkedToTestCases ?? 0;
    const bugsFoundPerRecording = metrics.bugsFoundPerRecording ?? 0;
    const recordingToReportConversionRate = metrics.recordingToReportConversionRate ?? 0;
    const totalStorageUsedGB = metrics.totalStorageUsedGB ?? 0;
    const avgUploadTime = metrics.avgUploadTime ?? 0;
    const recordingProcessingFailures = metrics.recordingProcessingFailures ?? 0;
    const activeRecordingUsers = metrics.activeRecordingUsers ?? 0;
    const recordingsPerUser = metrics.recordingsPerUser ?? 0;
    const recordingUsageGrowth = metrics.recordingUsageGrowth ?? 0;
    const recordingTrend = metrics.recordingTrend ?? [];
    ...
    const recordingTrendPercent = computeTrend(recordingTrend);
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center justify-center p-8">
                    ...
                    <span className="ml-2 text-foreground">Loading recording metrics...</span>
                </div>
            </div>
        );
    }
    ...
    // Debug: Log metrics to console if nothing displays (remove in production)
    console.log('Processed Recording Metrics:', metrics);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card rounded-lg shadow-theme-sm border border-border p-6">
                    <h3 className="font-semibold text-foreground mb-4 flex items-center">
                        <Link className="w-5 h-5 mr-2 text-success" />
                        Recording Effectiveness
                    </h3>
                    <div className="space-y-6">
                        <div className="text-center">
                            <div className="text-3xl font-bold text-info">{bugsFoundPerRecording.toFixed(1)}</div>
                            <p className="text-sm text-muted-foreground">Average bugs found per recording</p>
                        </div>
                        <div className="text-center">
                            <div className="text-3xl font-bold text-success">{recordingToReportConversionRate.toFixed(1)}%</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-card rounded-lg shadow-theme-sm border border-border p-6">
                    <h3 className="font-semibold text-foreground mb-4 flex items-center">
                        <Upload className="w-5 h-5 mr-2 text-info" />
                        Upload Performance
                    </h3>
                    <div className="space-y-4">
                        <div>
                ...
                <div className="bg-card rounded-lg shadow-theme-sm border border-border p-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="text-2xl font-bold text-destructive">{recordingProcessingFailures}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div className="space-y-2 text-sm">
                            {totalRecordings > 0 && recordingsWithNetworkData / totalRecordings < 0.7 && (
                            ...
                            {totalRecordings > 0 && recordingsWithConsoleData / totalRecordings < 0.8 && (
                                <div className="flex items-start space-x-2">
                                    <div className="w-2 h-2 bg-warning rounded-full mt-1.5"></div>
                                    <span className="text-muted-foreground">
                    ...
                    <div>
                        ...
                        <div className="space-y-2 text-sm">
                            ...
                            {recordingUsageGrowth !== 0 && (
                                <div className="flex items-start space-x-2">
                                    <div className="w-2 h-2 ${recordingUsageGrowth > 0 ? 'bg-success' : 'bg-destructive'} rounded-full mt-1.5"></div>
                                    <span className="text-muted-foreground">
                                        Usage {recordingUsageGrowth > 0 ? 'growing' : 'declining'} by {Math.abs(recordingUsageGrowth).toFixed(1)}% this period
                                    </span>
                                </div>
                            )}
                        </div>
