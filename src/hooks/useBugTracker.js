...
export const useBugTracker = ({ enabled = true, suite = null, user = null } = {}) => {
    ...
    const refetchBugs = useCallback(async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setBugs(bugsData);
                setFilteredBugs(bugsData); // Reset filteredBugs to match
                setError(null);
            } else {
    ...
    const updateBugInFirestore = useCallback(async (bugId, updates) => {
        ...
        try {
            ...
            if (error.message === 'Document not found') {
                ...
                await refetchBugs(); // Sync state with Firestore
            }
            return false;
        } finally {
            setIsUpdating(prev => {
            ...
            // Fallback to clear stuck isUpdating
            setTimeout(() => {
                setIsUpdating(prev => {
                    const newSet = new Set([...prev]);
                    if (newSet.has(bugId)) {
                        newSet.delete(bugId);
                        console.log('Cleared stuck isUpdating for:', bugId);
                    }
                    return newSet;
                });
            }, 5000);
        }
    }, [getBugsCollectionPath, refetchBugs]);
    ...
    const deleteBugs = useCallback(async (suiteId, bugIds) => {
        ...
        try {
            ...
            await Promise.all(bugIds.map(id => firestoreService.deleteDocument(`${bugsCollectionPath}/${id}`)));
